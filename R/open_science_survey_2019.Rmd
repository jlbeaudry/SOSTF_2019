...
valid_rep_est <- filter(df, RepEstimate_1 != "NA")
...
nvalid_crisis <- filter(df, crisis != "NA") %>% 
  nrow()
#no_crisis_perc <- filter(df, crisis == "No Crisis") %>% 
 # (100* ())

# messy code to give percentages, but it works & allows me to insert the percentages
  # into the text! :) 

c <- count(df$crisis) #frequencies per level
nvalid_crisis #total number of valid cases
p <- round(100*(c [,2] / nvalid_crisis),2) #gives percentages per level

# function to get percentages
perc <- function (x,y){  #x = variable; #y = nrows of valid cases from above
  c <- count (x)         #frequencies per level
  round (100*(c [,2] / y), 0) #forcing to 0 decimal places   
}

p <- perc(x = df$crisis, y = nvalid_crisis)  
...
crisis". Of the `r nvalid_crisis` respondents who answered this question, `r p[1]`% indicated that they didn't know if there was a crisis, `r p[2]`% indicated there was no crisis, `r p[3]`% indicated that there was a slight crisis, and `r p[4]`% that there was a significant reproducibility crisis in their field. 
...
# just an FYI that this does the same thing
#nvalid_os_exp <- df %>% 
