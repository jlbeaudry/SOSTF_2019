...
# Inserted between backtick r and backtick I think

```{r tables-mtcars, eval=TRUE}
#x3 <- df %>% 
  #table(df$OverallExperience) %>%
...
nvalid <- nrow(subset(df, !is.na(CodeExp)))
...
```{r ocode_experience_figure, echo=FALSE, warning=TRUE}
...
p <- ggplot(data = subset(df, !is.na(CodeExp)), aes(x=CodeExp)) + 
