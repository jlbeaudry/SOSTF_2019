...
# Am i missing it? I don't see the integer number, and I don't see what the "n = %d" is
  # doing....
...
# Extract only valid responses
piedata <- validdata[,'CodeExp']

# Create new index variable so we can order the data logically
piedata <- mutate(piedata,CodeExpLev = case_when(
  CodeExp == "Unaware"  ~ 0,
  CodeExp == "Aware, But Not Used"  ~ 1,
  CodeExp == "Some Use"  ~ 2,
  CodeExp == "Regular Use" ~ 3))

# Order the data
piedata <- arrange(piedata,CodeExpLev)

# Colors are not working in the pie chart: no idea why
colours = heat.colors(4, alpha = 1)
