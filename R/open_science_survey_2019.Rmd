...
perc <- function (x,y){  #x = variable; #y = n of valid cases from above
  c <- count (x)         #frequencies per level
  round (100*(c [,2] / y), 0) #rounding to 0 decimal places   
}

per_crisis <- perc(x = df$crisis, y = nvalid_crisis)  #percentage per level divided by total
  # number of valid cases for the crisis variable
...
crisis". Of the `r nvalid_crisis` respondents who answered this question, `r per_crisis[1]`% indicated that they didn't know if there was a crisis, `r per_crisis[2]`% indicated there was no crisis, `r per_crisis[3]`% indicated that there was a slight crisis, and `r per_crisis[4]`% that there was a significant reproducibility crisis in their field.   

Participants were also asked to estimate the percentage of research publications in 
their field that are reproducible. In the figure below, we plotted participants' 
reproducibility estimates according to their responses to the perceived reproducibility 
crisis in their field. The black bar is the mean estimate of reproducibility according to the 
perceived crisis. 


```{r crisis_by_estimate_stripchart, echo=FALSE, warning=TRUE}
...
#Right...I really like the idea of pulling out this stuff from the figure itself....
...
p <- ggplot(data = subset(df, !is.na(RepEstimate)), 
...
per_os_exp <- perc(x = df$OverallExperience, y = nvalid_os_exp)
...
This figure shows how many people have experience with any open science practices. Before we asked participants about their experience with open science practices, we explained that the practices enveloped by the umbrella term of "open science" were study preregistration, open materials and/or code, open data, pre-publication archiving, and open access publishing. Of the `r nvalid_os_exp`participants who answered this question, `r per_os_exp[4]`% (or, `r c_os_exp[4,2]` reported that they were unaware of open science practices; `r per_os_exp[1]`% reported that they were aware of open science practices, but had not used them; `r per_os_exp[3]`% reported that they had some experience with open science practices; and only `r per_os_exp[2]`% reported that they had extensive experience with open science practices. 

```{r os_experience_bar, echo=FALSE, warning=TRUE}

# TOM, THIS WILL NEED TO BE UPDATED WITH YOUR FUNCTION# 
