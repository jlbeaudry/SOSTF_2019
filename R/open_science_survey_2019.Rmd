...
# define functions here. Could be moved to a separate file I guess.

simple_pie_chart <- function(the_data,label_var="",the_title="",colours){
...
p <- plot_ly(pie_dataframe, labels = ~Var1, values = ~Freq, type = 'pie',
             textposition = "inside",
             ...
             hole=0.3,
             pull=0,
             title=list(text=the_title,position="top center",font=list(size=20, color="black")))
return(p)
}


simple_bar_graph <- function(the_data,label_var="",the_title="", yname="Percentage", xname="Response",bar_colour="blue",line_colour="",line_width=1){
  ...
  bar_data <- bar_data*100/nvalid
  
  # here we use formatting to insert an integer number into the title text
  the_title = sprintf(the_title,nvalid)
  
  bar_dataframe <- as.data.frame(bar_data)
  ...
  # create the plot
  p <- plot_ly(bar_dataframe, type="bar",
    x = ~Var1, y = ~Freq,
    marker = list(color = bar_colour,
                           line = list(color = line_colour, width = line_width))) %>%
    layout(title=the_title,yaxis = list(title=yname),xaxis = list(title=xname))
  return(p)
}
...
p <- simple_bar_graph(df,"CodeExp",the_title=title,xname = "What is your experience with open materials and/or code?", yname="Proportion",bar_colour = "orange",line_colour = "black")
p

# colours = heat.colors(4)
colours = rev(topo.colors(8))
p <- simple_pie_chart(df,"CodeExp",the_title=title,colours)
p
