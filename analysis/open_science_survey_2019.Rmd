...
the_data <- filter(the_data,CodeExp != "NA")
...
p <- plot_ly(the_data, labels = the_data[[label_var]], type = 'pie',
             textposition = "inside",
...
# this is the old attempt, which multiplied the values... 
# values = the_data[[sort_var]],
...
# Tom, I haven't figured out the issue yet, but I'm flagging for you that the 
  # numbers on this figure aren't correct. Print the object 'c2' above to see
  # the correct values. I have to run to a meeting, so I can't investigate now.

colours <- brewer.pal(4, "Blues")
p <- simple_pie_chart(df,label_var="CodeExp",the_title = title, colours = colours)
p
```
