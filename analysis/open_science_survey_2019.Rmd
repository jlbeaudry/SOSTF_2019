...
# If knit as a pdf, include width and height parameters in the plot_ly function
  #  width = 300, 
  #  height = NULL,
...
knitr::kable(col.names = c("FOR Code", "Field of Research Division", "Frequency",
                           "Percentage"), 
             caption = sprintf("Reported Two-Digit Field of Research Codes for Respondents (n = %d)", nrow(df))) %>% 
  kable_styling(bootstrap_options = "striped", full_width = F, position = 'center', 
                font_size = 14)
...
knitr::kable(col.names = c("FOR Code", "Field of Research Division", "Frequency"),
             caption = sprintf("Reported Two-Digit Field of Research Codes for Student Respondents (n = %d)", (sum(student[3])))) %>% 
...
knitr::kable(col.names = c("FOR Code", "Field of Research Division", "Frequency"),
             caption = sprintf("Reported Two-Digit Field of Research Codes for Staff Respondents (n = %d)", (sum(staff[3])))) %>% 
  kable_styling(bootstrap_options = "striped", full_width = F, position = 'center', 
                font_size = 14)
...
The figure below shows participants' experience with open science practices in general. 
Of the `r nvalid_os_exp` participants who answered this question, `r per_os_exp[4]`% 
(or, `r c_os_exp[4,2]` if we are going to use frequencies here) reported that they were
unaware of open science practices; `r per_os_exp[1]`% reported that they were 
aware of open science practices, but had not used them; `r per_os_exp[3]`% reported 
that they had some experience with open science practices; and only `r per_os_exp[2]`%
reported that they had extensive experience with open science practices. 
...
nvalid_os_exp <- sum(os_exp[2]) # I need this for the title
...
# CLEAN ALL OF THIS UP -- USE THE CODE FROM CODE_IMP
...
Of the `r nvalid_prereg_exp` participants who answered this question, `r per_prereg_exp[4]`% 
were unaware of study preregistration; `r per_prereg_exp[1]`% were aware of study 
preregistration, but had not used it; `r per_prereg_exp[3]`% had some experience 
with it, but did not regularly preregister their studies; and `r per_prereg_exp[2]`% 
regularly preregistered their studies. 
...
# check this order & use the other ordering approach rather than using descending
...
respondents who answered this question, `r b [7,2]` (`r b [7,3]`%) 
...
# plot the reordered data and colours. Because the data is in a tibble rather than a dataframe with
  # counts, I could use 'geom_col' rather than 'geom_bar'(or I could use stat = "identity" with
  # 'geom_bar')
...
# THE COMMENTED OUT CODE HELPS ME TO TROUBLESHOOT, BUT ULTIMATELY THE TB CODE
  # GETS ME WHAT I NEED IN THE END WITH A LOT LESS CODE --> I SHOULD GO BACK TO 
  # PREREG AND SIMPLIFY THAT CODE! 

# create objects with the counts for each valid response for the variable
  # I DON'T THINK I NEED THIS...
# c_code_imp <- plyr::count(valid_code_imp$CodeImp)
# c_code_imp_ex <- plyr::count(valid_code_imp_ex$CodeImp)
...
# calculate the percentage for each response
  # I DON'T THINK I NEED THIS...
# per_code_imp <- perc(x = valid_code_imp$CodeImp, y = nvalid_code_imp)
# per_code_imp_ex <- perc(x = valid_code_imp_ex$CodeImp, y = nvalid_code_imp_ex)
...
THIS ALL NEEDS TO BE RECODED FOR CODE RATHER THAN PREREG!

```{r code_imp_ex_pie_chart, echo = FALSE, warning = TRUE}

title = "In your opinion, how important for your field is it that materials and/or code are openly available (n = %d)"

# pie chart
colours <- brewer.pal(4, "Blues")
p <- simple_pie_chart(the_data = valid_code_imp_ex, label_var = "CodeImp", 
                      the_title = title, 
                      colours,the_quantity = "percent")
p
```
...
nvalid_code_imp_ex <- sum(tb_ex[2]) # I need this for the title
...
and/or materials. A total of `r nvalid_code_exp` answered this question.
...
```{r code_exp_table, echo=FALSE, warning=TRUE , eval=FALSE}

tb_c_exp <- valid_code_exp %>% 
  dplyr::count(CodeExp) %>%  
    mutate(Percentage = round (n/nvalid_code_exp*100))

tb_c_exp2 <- tb_c_exp [c(4,1,3,2),] # fix the order of responses
  ...
  dplyr::mutate(CodeConcern = factor (CodeConcern, 
                levels = c("Others might find errors in my published work", 
                        ...
                        "Others might find it difficult to understand my materials/code", 
                         ...
                         "No concerns")))
  # I don't get it. tb & tb2 look the same in the tibble, but the order is completely different
