...
black bar is the mean estimate of 
reproducibility by the perceived crisis levels. 

\newpage

`r fig_nums("crisis")`

```{r crisis_by_estimate_stripchart, fig.height=6, fig.width=12}
...
# `r sprintf(crisis.cap (n = %d), nvalid_rep_est)`
# `r sprintf(table_nums("crisis"),(n=%d),nvalid_rep_est)`
#how do I add in the sprintf?
```
...
```{r stacked_imp_bar, fig.height=6, fig.width=12}
...
```{r stacked_exp_bar, fig.height=6, fig.width=12}
...
# Perceived Barriers and Reported Concerns about Open Science Practices
...
fig_nums(name = "barriers", 
           caption = "Barriers to uptake of open science in your field", 
           display = FALSE)
```
...
```{r os_barrier_bar, fig.height = 6, fig.width=12}

# it's a really big graph, so I'm setting fig.height = 6, fig.width = 12 for html, 
...
# change base_size to 12 for regular knit & remove size = 14 from plot.title
```

For each of the four main open science practices, we also asked respondents about 
their concerns about each of the practices. 
...
```{r prereg_exp_pie, include=FALSE}
...
```{r prereg_concerns_bar, fig.height=6, fig.width=12}
...
```{r code_imp_ex_pie, include = FALSE}
...
```{r code_concerns_bar, fig.height=6, fig.width=12}
# build the bar chart
...
```{r data_concerns_bar, fig.height=6, fig.width=12}
...
```{r prepub_imp_ex_pie, eval=FALSE}
...
```{r prepub_concerns_bar, fig.height=6, fig.width=12}
...
```{r oa_fees_bar, fig.height=6, fig.width=12}
...
```{r os_most_bar, fig.height=6, fig.width=12}
