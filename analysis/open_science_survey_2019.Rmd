...
author: "Jennifer Beaudry, Tom Johnstone, Jordy Kaufman, Lisa Given & Michelle Vincent"
...
We, the core working group of the Swinburne Open Science Task Force, conducted 
a survey in September 2019 to examine our researchers' current use of open 
science practices, their perceptions of these practices, and their perceived 
barriers to adopting these practices. 

Across the university, `r n` 
...
of their responses to the various quantitative questions. We also asked a number
of open-ended questions, which we are currently analysing. The full survey is
available at https://osf.io/a4bwp/. 

## Definitions

TO SAVE US TIME LATER (JORDY TO DECIDE WHERE THESE GO)
...
Table \ref{tab:ac_level_table} reports the academic levels for all respondents. 
...
disciplinary groups (see Appendix 1 for details). Table \ref{tab:discipline_table} 
shows the breakdown by these disciplinary groups.
...
we plotted participants' reproducibility estimates according to their responses 
to the perceived crisis in their field (n = `r nvalid_rep_est`). The 
black bar is the mean estimate of reproducibility by the perceived crisis 
levels. \newline 
...
were (i) study preregistration, (ii) open materials and/or code, (iii) open data, 
(iv) pre-publication archiving, and (v) open access publishing.   

Table \ref{tab:tables_os_experience}  shows participants' experience with open 
science practices in general. Of the `r nvalid_os_exp` participants who answered 
this question, `r os_exp[3,3]`% reported that they were unaware of open 
science practices; `r os_exp[1,3]`% reported that they were 
aware of open science practices, but had not used them; `r os_exp[2,3]`%
reported that they had some experience with open science
practices; and only `r os_exp[4,3]`% reported that they had
...
NEED TO INTRODUCE THIS SECTION FOR THE VARIOUS PRACTICES...
