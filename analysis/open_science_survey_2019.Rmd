...
author: 
    - "Dr Jennifer Beaudry, A/Prof Jordy Kaufman, Prof Tom Johnstone," 
    - "Prof Lisa Given & Michelle Vincent"
...
### USE LATEX FONT FOR GGPLOT, PLUS CHANGE FONT SIZE
 ...
 # theme(plot.title = element_text(hjust = 0.5)) # centre the title if included
...
In late 2018, then Deputy Vice Chancellor - Research & Enterprise, Professor 
Aleks Subic, officially formed the Swinburne Open Science Task Force in order to 
generate recommendations for how Swinburne as an institution can most 
productively engage with open sciences practices. To best inform these 
recommendations the core working group of the Task Force surveyed our 
researchers’ current use of open science practices, their perceptions of these 
practices, and their perceived barriers to adopting these practices. Specifically, 
we were interested in their attitudes and experiences with (i) study 
preregistration, (ii) open materials and/or code, (iii) open data, (iv) 
pre-publication archiving, and (v) open access publishing. 
Over 200 researchers (n = 239) completed our online survey in September 2019. 
This document presents an overview of their responses to the various 
quantitative questions. The key finding was that researchers overall perceived 
most open science practices as important for their fields; however, most 
researchers engaged in these practices infrequently or not at all. Our data also 
revealed some of key barriers researchers felt impeded their use of open science 
practices.

# Methodology 

Across the university, `r length(df$ParticipantNumber)` researchers started the online survey 
(run through Qualtrics). Not all respondents completed 
all questions; thus, we report the sample size for each question. 
This document summarises their responses to the various quantitative questions. 
We also asked a number of open-ended questions, which we are currently 
analysing. The full survey is available at https://osf.io/a4bwp/. 

## Definitions

In the survey, we explained that the umbrella term "open 
science" ensapsulated various practices, including (i) study preregistration, (ii)
open materials and/or code, (iii) open data, (iv) pre-publication archiving, and
(v) open access publishing. As they progressed through the survey, we also offered 
more detailed definitions of each of these open science practices. 

### Preregistration

We defined study preregistration as documenting and submitting
...
We defined open materials and/or code as researcher-created resources used 
while collecting or analyzing data (e.g., survey questions, 
...
We used Foster's definition of open data: "online, free of cost, accessible 
data that can be used, reused and distributed provided that the data source 
is attributed and shared alike" (Foster Open Science). 

### Pre-publication archiving

We definied pre-publication archiving as making a manuscript openly available 
*before* it undergoes peer review in an academic journal or other outlet.
Generally, this is achieved by uploading the manuscript to an archive such as arXiv
(physics, maths), bioRxiv (biology), SocArxiv (sociology), etc. We explained that
pre-publication archiving is also known as preprint archiving. 

### Open Access Publishing

Finally, we explained that there are two basic models for open access publishing. 
First, researchers could pay their publisher a fee (an Article Processing Charge) 
to make the *final version of record* available for open access in a 
peer-reviewed journal. Second, researchers could publish under a normal contract 
in a peer-reviewed journal, and deposit the *final accepted version of the manuscript* 
(peer-reviewed but not journal-formatted) in an open access repository 
(e.g., institutional repository or an external subject repository). 
...
ask about their academic level and their Discipline (based on 2-digit and 
4-digit Field of Research codes). 

## Academic Levels 

```{r demographics_ac_level}
...
# for pdf, include Overview as a heading. Remove for html so I can see the full 
  # table of contents
...
Table \ref{tab:ac_level_table} reports the academic levels for respondents who 
answered the questions; 97 respondents chose not to report their academic level.
...
knitr::kable(a3, booktabs = T, linesep = "",
             col.names = c("Academic Levels", "Frequency", "Percentage"), 
...
FOR_freq <- df %>% 
  select (FORcode_2_num_0, FORcode_2_label) %>% 
  dplyr::count(FORcode_2_num_0, FORcode_2_label) %>% 
  arrange(-desc(FORcode_2_num_0))
...
In terms of Field of Research [FOR] codes, `r a` respondents classified themselves 
...
we combined similar divisions with smaller numbers of respondents into 
disciplinary groups (see Table \ref{tab:discipline_table}). See Appendix 1 for 
details of how we combined FOR codes into disciplinary groups. 
...
knitr::kable(booktabs = T, linesep = "",
             col.names = c("Disciplines", "Frequency", "Percentage"), 
...
fig_nums(name = "crisis", 
           caption = "Estimates of reproducibility by perceived crisis in field", 
           display = FALSE)
# crisis.cap <- table_nums("crisis") # needs to be separate steps
```

# Estimates of Reproducibility by Perceived Crisis  

In line with Baker (2016), we asked participants if they believed their field 
is experiencing a "reproducibility crisis". Of the `r nvalid_crisis` respondents 
who answered this question, `r per_crisis[1]`% indicated that they didn't know 
if there was a crisis, `r per_crisis[2]`% indicated there was no crisis, 
`r per_crisis[4]`% indicated that there was a slight crisis, and 
`r per_crisis[3]`% that there was a significant reproducibility crisis in their 
field.  

\newpage

We also asked participants to estimate the percentage of research publications in 
their field that are reproducible. In `r fig_nums("crisis", display = "cite")`,
we plotted participants' reproducibility estimates according to their perceived 
crisis in their field (n = `r nvalid_rep_est`). The black bar is the mean 
estimate of reproducibility by the perceived crisis levels. \newline 

```{r crisis_by_estimate_stripchart}
...
# save it in the figs folder. 
ggsave(here::here("analysis", "figs", "reprod_estimates_by_crisis.png"), plot = p, 
              dpi = "retina")
...
```{r crisis, out.width = '100%'}
knitr::include_graphics("figs/reprod_estimates_by_crisis.png")
```


# Views and Use of Open Science Practices
...
Before asking participants about individual open science practices, we asked 
about their experience with open science practices, in general (see 
Table \ref{tab:tables_os_experience}). Of the `r nvalid_os_exp` participants who 
answered this question, `r os_exp[3,3]`% reported that they were unaware of open 
science practices and only `r os_exp[4,3]`% reported that they had
extensive experience with open science practices. 

```{r tables_os_experience}

tb_os_exp <-os_exp [c(3,1,2,4),] # rearrange the responses

tb_os_exp %>% 
knitr::kable(booktabs = T, linesep = "",
             col.names = c("Reported Experience", "Frequency", "Percentage"), 
...
## Perceived Importance of Open Science Practices

```{r stacked_imp_data, warning=FALSE}
...
fig_nums(name = "imp_all", 
           caption = "Perceived importance of open science practices", 
           display = FALSE)

```

Throughout the survey, respondents separately rated the four main open science 
practices in terms of how important they are for their field. Specifically, 
we asked them about (i) preregistering their studies in advance of data collection, 
(ii) making their study material and/or code openly available, (iii) making their 
data openly available, and (iv) making their work available on pre-publication archives. 

`r fig_nums("imp_all", display = "cite")` illustrates the responses from the `r total_n` 
respondents who answered the perceived importance question for at least one of 
the four practices. Open materials and/or code and open data were 
the most important practices for respondents' fields; nearly 75% indicated 
that these practices were important (either extremely important or somewhat 
important) for their field. Pre-publication archiving was the least important
practice; only 36% indicated that it was important for their field. \newline


```{r stacked_imp_bar}
# Relevel my importance factors

tb_imp$Importance <- factor (tb_imp$Importance, levels = c("Extremely important",
                                           "Somewhat important", 
                                           "Somewhat unimportant",
                                           "Not at all"))
tb_imp$Practice <- factor (tb_imp$Practice, levels = c("PrePubImp", 
                                               "DataImp", 
                                               "CodeImp", 
                                               "PreRegImp"))
...
# title = str_wrap (sprintf("Perceived importance of open science practices (n = %d)", 
                          # total_n), width = 40)

p <- ggplot(tb_imp, aes(fill = Importance, y = Percentage, x = Practice)) +
...
# save it in the figs folder. 
ggsave(here::here("analysis", "figs", "all_importance.png"), plot = p, 
              dpi = "retina")
...
```{r imp_all, out.width = '100%'}
knitr::include_graphics("figs/all_importance.png")
```


```{r stacked_imp_table, include=FALSE}
...
tb_imp %>% 
knitr::kable (booktabs = T, linesep = "",
              col.names = c("Practice", "Perceived Importance", "Frequency",
                            "Percentage"), 
              caption = 
                sprintf("Perceived Importance of Open Science Practices (n = %d)",
                                total_n), 
              align = 'lcc') %>% 
  styler_pdf() 
...
```{r stacked_exp_data, warning=FALSE}
...
fig_nums(name = "exp_all", 
           caption = "Reported experience with open science practices", 
           display = FALSE)
```

## Experience with Open Science Practices

Participants also reported their own experience with each of the four
main open science practices: (i) preregistering their studies in 
advance of data collection, (ii) making their study material and/or code openly 
available, (iii) making their data openly available, and (iv) making their work 
available on pre-publication archives. 

`r fig_nums("exp_all", display = "cite")` includes the data from the `r total_n` 
respondents who answered the experience question for at least one of the four 
practices. Our respondents reported very little experience with any of these open 
science practices. Respondents had the most experience with open materials/code 
and open data, but even with those practices, the most common response (37% for materials/code and
41% for data) was that they were aware of the practice, but had not personally 
used it. With regard to preregistration, respondents were largely unaware of 
the practice (39%) or were aware, but had not used it (38%). Only 16% of respondents
reported any experience with pre-publication archiving. \newline

```{r stacked_exp_bar}
# Relevel my Experience factors
# Different labels used for various practices in the survey, so I need to relabel

tb_exp <- tb_exp %>%  dplyr::mutate(Experience = fct_recode (Experience, 
                            "Extensive Experience" = "Regular Use",
                            "Extensive Experience" = "Reg Use",
                            "Some Experience" = "Some Use"))

tb_exp$Experience <- factor (tb_exp$Experience, levels = c("Extensive Experience",
                                           "Some Experience", 
                                           "Aware, But Not Used",
                                           "Unaware"))
tb_exp$Practice <- factor (tb_exp$Practice, levels = c("PrePubExp", 
                                               "DataExp", 
                                               "CodeExp", 
                                               "PreRegExp"))
...
# title = str_wrap (sprintf("Reported experience with open science practices (n = %d)",
                          # total_n), width = 40)


p <- ggplot(tb_exp, aes(fill = Experience, y = Percentage, x = Practice)) +
  geom_bar(position="fill", stat="identity", colour = "black") +
  scale_fill_brewer(palette = "Blues", na.value = "grey50", direction = -1, 
...
ggsave(here::here("analysis", "figs", "all_experience.png"), plot = p, 
       dpi = "retina")
```
...
```{r exp_all, out.width = '100%'}
knitr::include_graphics("figs/all_experience.png")
```

For participants who reported some experience or extensive experience with 
each of the four main open science practices, we asked them follow up questions 
to better understand how they have used the practice in their research activities. 

### Use of Study Preregistration
...
PreRegUse_long <- df %>% 
  select (c("ParticipantNumber", "AcLevel_Label",
            "PreRegUse_clinicalTrial":"PreRegUse_other",
            "FORcode_2_label")) %>%  
  pivot_longer(c("PreRegUse_clinicalTrial":"PreRegUse_other"),
               names_to = "PreRegUse", 
               values_to = "PreRegUse_endorsement") %>% 
  filter(PreRegUse_endorsement == "1")
...
c <- metadata %>% 
    filter(Scale == "PreRegUse") %>% 
    filter (NewVariable != "PreRegUse_text")

d <- full_join(PreRegUse_long, c, by = c("PreRegUse" = "NewVariable")) %>% 
  rename ("PreRegUse_label" = "ItemText")

# getting a tibble for the counts
c_prereg_use <- plyr::count(d$PreRegUse)

# to figure out how many respondents shared concerns calculate unique number of participants
n_prereg_use <- length(unique(d$ParticipantNumber))

# create a tibble with percentages (of respondents who answered this question) & 
# then reorder them by descending percentages
tb <- d %>% 
  dplyr::count(PreRegUse_label) %>% 
  mutate(Percentage = round (n/n_prereg_use*100)) %>% 
  dplyr::arrange(desc(n)) 

```

For those participants who reported that they preregister their studies, we asked 
them for more information about how they have used preregistration in their 
previous work. Respondents were allowed to select as many uses as applied. 
Of these `r n_prereg_use` researchers, the most popular response (`r tb[1,3]`%) was 
that they had preregistered at least one study that was *not* a clinical trial.
A small percentage of respondents (`r tb[5,3]`%) reported that they had submitted
a Registered Report---a new type of manuscript format where the proposed method 
and analytic plan are peer-reviewed *__before__* data collection (for more information
see the [Center for Open Science](https://www.cos.io/our-services/registered-reports)). 
...
tb %>%
  knitr::kable (
    booktabs = T,
    linesep = "",
    col.names = c("Reported Use", "Frequency", "Percentage"),
    caption = sprintf("Reported Use of Study Preregistration (n = %d)", n_prereg_use),
    align = 'lcc'
  ) %>%
  styler_pdf() 
```
...
# Perceived Barriers and Reported Concerns

## Barriers to Engaging in Open Science Practices
...
fig_nums(name = "barriers", 
           caption = "Barriers to the uptake of open science in your field", 
           display = FALSE)
```

We provided the respondents with a list of possible barriers to the uptake of
open science practices in general. We asked them to endorse any statement that 
they agreed is a barrier in their field. `r fig_nums("barriers", display = "cite")` 
shows the percentage of the `r n_osbarrier` respondents who selected each 
response. \newline

```{r os_barrier_bar, fig.height=8}
...
tb2 <- tb %>% 
  dplyr::mutate(OSBarrier_label = factor (OSBarrier_label,
                                levels = c("Other", #TOP STARTS AT BOTTOM
                                "The OS community is intimidating",
                                "Researchers don't want to be told how to do their research",
                               "Researchers are discouraged by colleagues",
                               "HDR students are discouraged by thesis supervisors",
                               "Lack of interest from researchers",
                               "Lack of supporting infrastructure",
                               "Lack of expertise (e.g., assignment of metadata)", 
                               "Lack of mandates from funders, institutions, or regulators",
                               "Lack of time to learn",
                               "Lack of time to engage",
                               "Lack of information",
                               "Lack of recognition in my field about the value of OS",
                               "Lack of professional staff that provide support",
                               ...
                               "Lack of funding for open access publishing",
                                "I do not perceive any barriers"))) #BOTTOM IS TOP
...
# build the bar chart
# title = str_wrap (sprintf("Barriers to uptake of open science in your field (n = %d)",
                          # n_osbarrier), width = 40)
...
p <- ggplot(tb2) + 
  ...
  theme(plot.title = element_text(hjust = 0.5, size = NULL)) 
# p

ggsave(here::here("analysis", "figs", "all_barriers.png"), plot = p,
       dpi = "retina")
```

`r fig_nums("barriers")`

```{r barriers, out.width = '100%'}
knitr::include_graphics("figs/all_barriers.png")
```


For each of the four main open science practices, we also asked respondents about 
their concerns about each of the practices. For each of the practices, we asked
respondents to share their concerns in a free text format. Then, we provided 
a list of possible concerns that researchers could have about the relevant practice. 
From this list, respondents were able to select as many concerns as were relevant.

## Concerns about Preregistration 

```{r prereg_concerns_data}
...
fig_nums(name = "prereg_concerns", 
           caption = "Concerns about preregistering studies", 
           display = FALSE)

```

Of the `r n_prereg_con`
respondents who answered the question about their concerns for preregistering 
research studies, `r tb [7,3]`% reported that they do not share any of the listed 
concerns. `r fig_nums("prereg_concerns", display = "cite")` 
presents, of those who answered this question, the percentage who selected 
each of the concerns.  \newline

```{r prereg_concerns_bar}
...
# create a character vector for the colours for each bar
cols <- c("lightskyblue3", "lightskyblue3", "lightskyblue3", "lightskyblue3", 
          "lightskyblue3", "lightskyblue3", "grey50", "lightskyblue3", "lightskyblue3")

# build the bar chart
# title = str_wrap (sprintf("Concerns about preregistering studies (n = %d)", 
                          # n_prereg_con), width = 37)
...
# save it in the figs folder. 
ggsave(here::here("analysis", "figs", "prereg_concerns.png"), plot = p, 
              dpi = "retina")
...
`r fig_nums("prereg_concerns")`


```{r prereg_concerns, out.width = '100%'}

knitr::include_graphics("figs/prereg_concerns.png")

```

## Concerns about Open Materials and/or Code

```{r code_concerns_data}
...
fig_nums(name = "code_concerns", 
           caption = "Concerns about open materials and/or code", 
           display = FALSE)
```

A total of `r n_code_con` respondents answered the question about their concerns 
about making research materials and/or code open. Of those who responded, 
`r tb [4,3]`% reported that they do not share any of the listed concerns. 
`r fig_nums("code_concerns", display = "cite")` presents, of those who answered 
this question, the percentage who selected each of the concerns.  \newline
...
# create a character vector for the colours for each bar
cols <- c("lightskyblue3", "lightskyblue3", "lightskyblue3", "grey50", 
          "lightskyblue3", "lightskyblue3", "lightskyblue3", "lightskyblue3", 
          "lightskyblue3", "lightskyblue3")

# build the bar chart
# title = str_wrap (sprintf("Concerns about open materials and/or code (n = %d)", 
                          # n_code_con), width = 40)
...
ggsave(here::here("analysis", "figs", "code_concerns.png"), plot = p,
       dpi = "retina")

```

`r fig_nums("code_concerns")`

```{r code_concerns, out.width = '100%'}
knitr::include_graphics("figs/code_concerns.png")

```

## Concerns about Open Data

```{r data_concerns_data}
...
fig_nums(name = "data_concerns", 
           caption = "Concerns about open data", 
           display = FALSE)
```

A total of `r n_data_con` respondents answered this question about concerns 
they had about making their data open. Of those who respondended, `r tb2 [11,3]`%
reported that they did not share any of the listed 
concerns. `r fig_nums("data_concerns", display = "cite")` presents, of those who 
answered this question, the percentage who selected each of the concerns.\newline
...
cols <- c("lightskyblue3", "lightskyblue3", "lightskyblue3", "lightskyblue3", 
          "lightskyblue3", "lightskyblue3", "lightskyblue3", "lightskyblue3", 
          "lightskyblue3", "lightskyblue3", "grey50", "lightskyblue3")

                    
# build the bar chart
# title = str_wrap (sprintf("Concerns about open data (n = %d)", 
                          # n_data_con), width = 40)
...
ggsave(here::here("analysis", "figs", "data_concerns.png"), plot = p,
       dpi = "retina")

```

`r fig_nums("data_concerns")`

```{r data_concerns, out.width = '100%'}
knitr::include_graphics("figs/data_concerns.png")

```

## Concerns about Pre-publication Archiving

```{r prepub_concerns_data}
...
fig_nums(name = "prepub_concerns", 
           caption = "Concerns about pre-publication archiving", 
           display = FALSE)
```

A total of `r n_prepub_con` respondents answered this question about concerns 
they had about uploading a manuscript to a pre-publication archive before 
submitting it for peer review. Of those who respondended,
`r tb [6,3]`% reported that they do not share any of the listed 
concerns. `r fig_nums("prepub_concerns", display = "cite")` presents, of those 
who answered this question, the percentage who selected each of the concerns. \newline 



```{r prepub_concerns_bar}
...
# build the bar chart
# title = str_wrap (sprintf("Concerns about pre-publication archiving (n = %d)", 
                          # n_prepub_con), width = 40)
...
ggsave(here::here("analysis", "figs", "prepub_concerns.png"), plot = p,
       dpi = "retina")
...
`r fig_nums("prepub_concerns")`


```{r prepub_concerns, out.width = '100%'}
knitr::include_graphics("figs/prepub_concerns.png")
...
publications from the last 5 years are open access (see Table 
\ref{tab:oa_prop_table}). Of the `r sum(tb[2])` 
participants who answered this question, the most common responses were that 
some (`r tb[1,3]`%) or none (`r tb[2,3]`%) of their 
publications were open access. A small proportion (`r tb[3,3]`%) 
indicated that they did not know what proportion were open access.
...
tb2 %>% 
knitr::kable (booktabs = T, linesep = "",
              col.names = c("Response", "Frequency", "Percentage"), 
...
fig_nums(name = "oa_fees", 
           caption = "How respondents paid for open access publishing", 
           display = FALSE)
```

We asked respondents, except those who said that none of their publications 
...
select as many concerns as they liked. `r fig_nums("oa_fees", display = "cite")` 
presents, of those who answered this question (n = `r n_oafees`), the 
percentage who used each of the different payment approaches.  \newline
...
# build the bar chart
# title = str_wrap (sprintf("How did you pay any Open Access publishing fees? (n = %d)",
                          # n_oafees), width = 40)
...
ggsave(here::here("analysis", "figs", "oa_fees.png"), plot = p,
       dpi = "retina")
...
```{r oa_fees, out.width = '100%'}
knitr::include_graphics("figs/oa_fees.png")
...
fig_nums(name = "os_most", 
           caption = "Most common open science practices in respondents' fields", 
           display = FALSE)

```

We asked respondents which of the following open science practices (if any) were
commonly used in their field. They could tick all that applied. 
`r fig_nums("os_most", display = "cite")`represents the percentage of the 
`r n_osmost` respondents who selected each response. Given that information about 
some of these open science practices are publicly available
and often associated with the published product (e.g., the final paper 
regularly includes links to repository storing data, materials, and/or code, as 
well as a link to the preregistration, if relevant), it is disconcerting that nearly
a fifth of respondents reported that they did not know about the open science 
...
# create a character vector for the colours for each bar
cols2 <- c("lightskyblue3", "lightskyblue3", "lightskyblue3",
          "lightskyblue3", "lightskyblue3", "grey50", 
          "lightskyblue3", "grey50", "lightskyblue3")
                    
# build the bar chart
# title = str_wrap (sprintf("Most common practices in respondent's field (n = %d)",
                          # n_osmost), width = 40)

p <- ggplot(tb2) + 
  aes(x = OSmost_label, y = Percentage) +
  geom_bar(stat = "identity", colour = "black", fill = cols2) +
  labs (x = element_blank(),
        y = "Percentage") +
  coord_flip(ylim = c(0,70)) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 50)) + #wrap the labels
...
ggsave(here::here("analysis", "figs", "os_most.png"), plot = p,
       dpi = "retina")
...
```{r os_most, out.width = '100%'}
knitr::include_graphics("figs/os_most.png")
...
## Appendix 1: Discipline Breakdown

```{r FOR_table_by_role, include = FALSE}
...
knitr::kable(booktabs = T, linesep = "",
             col.names = c("FOR Code", "Field of Research Division", "Frequency"),
...
knitr::kable(booktabs = T, linesep = "",
             col.names = c("FOR Code", "Field of Research Division", "Frequency"), caption = sprintf("Reported Two-Digit Field of Research Codes for Unknown Academic Levels (n = %d)",(sum(unknown[3])))) %>% 
  styler_pdf() 

```

As mentioned in Demographics, we merged FOR disciplines into disciplinary groups. 
Specifically, the new discipline grouping of Arts, Social Sciences, and Humanities 
(ASSH) included Education (FOR `r FOR_freq[11,1]`, 
n = `r FOR_freq[11,3]`); Studies 
in Human Society (FOR `r FOR_freq[14,1]`, n = `r FOR_freq[14,3]`); 
Studies in Creative Arts and Writing (FOR `r FOR_freq[17,1]`, n =
`r FOR_freq[17,3]`); and Language, Communication and Culture (FOR 
`r FOR_freq[18,1]`, n = `r FOR_freq[18,3]`). 

The new discipline grouping of Business & Law included Economics 
(FOR `r FOR_freq[12,1]`, n = `r FOR_freq[12,3]`); Commerce, Management, 
Tourism and Services (FOR `r FOR_freq[13,1]`, n = `r FOR_freq[13,3]`), and 
Law and Legal Studies (FOR `r FOR_freq[16,1]`, n = `r FOR_freq[16,3]`). 

The new discipline grouping of Technology & Computer Sciences included Information
and Computing Sciences (FOR `r FOR_freq[6,1]`, n = `r FOR_freq[6,3]`),
and Technology (FOR `r FOR_freq[8,1]`, n = `r FOR_freq[8,3]`). We 
allocated Built Environment and Design (FOR `r FOR_freq[10,1]`, n = 
`r FOR_freq[10,3]`) to the "Other" category. 
  
We retained the FOR divisions of Engineering (FOR `r FOR_freq[7,1]`, n 
= `r FOR_freq[7,3]`); Medical and Health Sciences (FOR `r FOR_freq[9,1]`,
n = `r FOR_freq[9,3]`); Physical Sciences (FOR `r FOR_freq[2,1]`,
n = `r FOR_freq[2,3]`); and Psychology and Cognitive Sciences (FOR 
`r FOR_freq[15,1]`, n = `r FOR_freq[15,3]`) as their own discipline 
groupings. Table \ref{tab:FOR_table} presents the breakdown for the number of 
respondents according to discpline groupings.

```{r FOR_table}

d2 <- df %>% 
  select (c(FORcode_2_num_0, FORcode_2_label)) %>% 
  dplyr::count(FORcode_2_num_0,FORcode_2_label) %>% 
  mutate (Percentage = round (n/nrow(df)*100))
...
dplyr::arrange(d2,-desc(FORcode_2_num_0)) %>% 
knitr::kable(booktabs = T, linesep = "",
             col.names = c("FOR Code", "Field of Research Division", "Frequency",
                           "Percentage"), 
...
## Appendix 2: Perceived Importance Tables
...
```{r prereg_imp_ex_table}

tb_ex2 <- tb_ex [c(2,4,1,3),]  # rearrange the responses

tb_ex2 %>% 
knitr::kable (booktabs = T, linesep = "",
              col.names = c("Perceived Importance", "Frequency",
                            "Percentage"), 
              caption = sprintf("Perceived Importance of Preregistration (n = %d)",
...
tb_ex2 %>% 
knitr::kable (booktabs = T, linesep = "",
              col.names = c("Perceived Importance", "Frequency", "Percentage"), 
...
\newpage
## Appendix 3: Reported Experience Tables
...
knitr::kable (booktabs = T, linesep = "",
              col.names = c("Reported Experience", "Frequency", "Percentage"),
              caption = sprintf("Experience with Study Preregistration (n = %d)",
                                nvalid_prereg_exp), 
              align = 'lcc') %>% 
  styler_pdf() 
...
Baker, M. (2016). 1,500 scientists lift the lid on reproducibility. *Nature News, 533*
(7604). Retrieved from https://www.nature.com/news/1-500-scientists-lift-the-lid-on-reproducibility-1.19970. 

Center for Open Science (nd). Registered Reports. Retrieved from https://www.cos.io/our-services/registered-reports

Foster Open Science (nd). Open data definition. Retrieved from https://www.fosteropenscience.eu/taxonomy/term/110. 
