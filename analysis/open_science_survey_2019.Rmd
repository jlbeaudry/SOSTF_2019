...
simple_pie_chart <- function (the_data, label_var="", the_title="", colours, order = "ByRespNum", the_quantity = "", include_NA = "NO"){
  ...
  pie_data <- count(the_data,vars=c(label_var,item_var))
...
simple_bar_graph <- function (the_data, label_var = "", the_title = "", 
  ...
  # tally the responses for each response option
  bar_data <- count(the_data,vars=c(label_var,item_var))
...
FOR_freq <- count(df$FORcode_2_label)
```
...
person indicated "Other", and `r FOR_freq[20,2]` respondents did not specify their 
FOR code. We did not have any responses from researchers in XXXX (04), XXXX (07), 
History and Archeology (21) and Philosophy and Religious Studies (22). 
...
f2$FORcode_2_label <- str_to_title(f2$FORcode_2_label) #change capilatlisation

dplyr::arrange(f2,-desc(FORcode_2_num)) %>% 
knitr::kable(col.names = c("FOR Code", "Field of Research", "Frequency", "Percentage"), caption = sprintf("Reported Two-Digit Field of Research Codes for Respondents (n = %d)",nrow(df))) %>% 
  kable_styling(bootstrap_options = "striped", full_width = F, position = 'center')


# Now look at how the data changes according to their academic level (student or not)
#  very few unspecified! Out of the 54 students, only 1 'other' and 1 'unspecified'

phd <- dplyr::filter(df, AcLevel_Label == "PhD Student" | AcLevel_Label == "Masters Student" | AcLevel_Label == "Research Assistant") %>% 
  dplyr::count(FORcode_2_label)

```

For ease of interpretation and to reduce the identifiability of the respondents, we classified similar FOR codes with a small number of respondents (less than 10) into disciplinary groups. Specifically, the new discipline grouping of Arts, Social Sciences, and Humanities (ASSH) included Education (13, <em>n</em> = `r FOR_freq[6,2]`), Studies in Human Society
(16, <em>n</em> = `r FOR_freq[18,2]`), Studies in Creative Arts and Writing 
(19, <em>n</em> = `r FOR_freq[17,2]`), and Language, Communication and Culture 
(20, <em>n</em> = `r FOR_freq[10,2]`). The new discipline grouping of Business & 
Law included Economics (14, <em>n</em> = `r FOR_freq[5,2]`), Commerce, Management, 
Tourism, and Services (15, <em>n</em> = `r FOR_freq[4,2]`), and Law and Legal 
Studies (18, <em>n</em> = `r FOR_freq[11,2]`). We allocated Built Environment and Design 
(12, <em>n</em> = `r FOR_freq [2,2]`) to the "Other" category. The new discipline grouping of 
Technology & Computer Sciences included Information and Computing Sciences 
(08, <em>n</em> = `r FOR_freq[9,2]`), and Technology (10, <em>n</em> = `r FOR_freq[19,2]`). 
  
We retained the FOR codes of Engineering (09, <em>n</em> = `r FOR_freq[7,2]`), 
Medical & Health Sciences (11, <em>n</em> = `r FOR_freq[13,2]`), Physical Sciences 
(02, <em>n</em> = `r FOR_freq[15,2]`), and Psychology & Cognitive Sciences 
(17, <em>n</em> = `r FOR_freq[16,2]`) as their own discipline groupings. The 
breakdown for the number of respondents according to discpline groupings is displayed in the next table.
  
<em>I AM SURE YOU WILL ALL AGREE THAT WE DON'T NEED THE INFO IN THE TEXT, BUT LEAVING
IT FOR NOW TO MAKE SURE WE UNDERSTAND HOW EVERYTHING WAS GROUPED TOGETHER.</em> :)
...
```{r prereg_pie_chart, echo = FALSE, warning = TRUE}
