...
# create a tibble with count & percentages (of respondents who answered this question) & 
...
reported that they do not share any of the listed concerns. The following table presents
the percentage of respondents who selected each of the concerns provided. 

```{r prereg_concerns_bar, echo=FALSE, warning=TRUE}

# need to reorder the response options...
b2 <- b %>% 
  dplyr::mutate(PreRegConcern = factor (PreRegConcern, 
                                levels = c("More difficult to find significant results", "More difficult to publish in certain journals", "Delays data collection", "Stifles creativity",  "Prevents exploratory research", "Need to look at data to analyse it", "Risk of scooping", "No concerns")))

# create a character vector for the colours for each bar
cols <- c("lightskyblue3", "lightskyblue3", "lightskyblue3", "lightskyblue3", "lightskyblue3", "lightskyblue3", "lightskyblue3", "grey50")
...
and/or code. Respondents were able to select as many concerns as they liked. A total of `r length(n_code_con)` respondents answered this question. Of those who respondended to this question,
`r code_con[4,2]` reported that they do not share any of the listed concerns. 
