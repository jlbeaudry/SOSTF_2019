...
```{r prereg_imp_ex_pie, echo = FALSE, warning = TRUE}
...
```{r prereg_exp_pie, echo = FALSE, warning = TRUE}
...
```{r code_imp_ex_pie, echo = FALSE, warning = TRUE}
...
```{r data_imp_ex_pie, echo = FALSE, warning = TRUE}
...
# AH...I only need these next two steps if I'm actually able to figure out how 
  # to use the `x` variable to reorder the variable, while using the `x_text`
  # variable to name the actual columns in the tables. Otherwise, skip this.
...
```{r prepub_imp_ex_pie, echo = FALSE, warning = TRUE}
...
```{r prepub_exp_pie, echo = FALSE, warning = TRUE}
...
d <- full_join(PrePubUse_long, c, by = c("PrePubUse" = "NewVariable")) %>% 
  rename ("PrePubUse_text" = "ItemText")
...
d2 <- d %>% # create tibble with the Data Use counts
  select(PrePubUse, PrePubUse_text)  %>% 
...
d <- full_join(PrePubConcern_long, c, by = c("PrePubConcern" = "NewVariable")) %>% 
  rename ("PrePubConcern_text" = "ItemTextShort")
...
# then reorder them by descending percentages
tb <- d %>% 
  dplyr::count(PrePubConcern_text) %>% 
...
d2 <- d %>% # create tibble with the Data Concern counts
  select(PrePubConcern, PrePubConcern_text)  %>% 
      dplyr::count(PrePubConcern) 
  #  mutate_if(is.character, ~factor(.))
...
# reorder the rows according to the PrePubConcern_text 
tb2 <- tb %>% 
  dplyr::mutate(PrePubConcern_text = factor(PrePubConcern_text, 
                levels = c("Other", 
...
# build the bar chart
title = sprintf("Concerns about pre-publication archiving (n = %d)", n_prepub_con)

p <- ggplot(tb2) + 
  aes(x = PrePubConcern_text, y = Percentage) +
  geom_bar(stat = "identity", colour = "black", fill = cols) +
  labs (x = element_blank(),
        y = "Percentage", 
        title = title) +
...
```{r oa_prop_pie, echo = FALSE, warning = TRUE}
...
```{r oa_fees_bar, echo=FALSE, warning=TRUE, eval=TRUE}
