...
their perceived barriers to using these practices. Across the university, `r n` started the survey, although not all respondents completed all questions; thus, 
...
```{r os_experience_pie_assh, echo = FALSE}
...
# create object with the counts for each valid response for this variable
c_prereg_imp <- count(valid_prereg_imp$PreRegImp)

# calculate number of rows with a valid response for this variable
nvalid_prereg_imp <- nrow(valid_prereg_imp)

# calculate the percentage for each response
per_prereg_imp <- perc(x = df$PreRegImp, y = nvalid_prereg_exp)

```


Of the `r nvalid_prereg_imp` participants who answered this question, `r per_prereg_imp[1]`% indicated that preregistration was extremely important for their field; `r per_prereg_imp[4]`% indicated that it was somewhat important; `r per_prereg_exp[5]`% indicated that preregistration was somewhat unimportant; and `r per_prereg_exp[2]`% indicated that it was not at all important for their field. A small number (`r per_prereg_exp[3]`%) indicated that researchers in their 
discipline do not conduct research studies. 

```{r prereg_imp_pie_chart, echo = FALSE, warning = TRUE}

title = "In your opinion, how important for your field is it that researchers preregister their studies (n = %d)"

# pie chart
colours <- brewer.pal(4, "Blues")
p <- simple_pie_chart(the_data = df, label_var = "PreRegImp", 
                      the_title = title, 
                      colours,the_quantity = "percent")
p
```

```{r prereg_imp_pie_chart_assh, echo = FALSE, warning = TRUE, eval=FALSE}
...
```{r prereg_imp_table, echo=FALSE, warning=TRUE}

p2 <- valid_prereg_imp %>% 
  dplyr::count(PreRegImp) %>% 
  mutate(Percentage = round (n/nvalid_prereg_imp*100))

dplyr::arrange(p2, desc(n)) %>% 
knitr::kable (col.names = c("Perceived Importance of Preregistration", "Frequency", "Percentage"), caption = sprintf("Perceived Importance of Preregistration (n = %d)", nvalid_prereg_imp)) %>% 
    kable_styling(bootstrap_options = "striped", full_width = F, position = 'center')
...
con <- count(PreRegCon_long$PreRegConcern)

# need the data to figure out how many respondents shared concerns
  # figure out unique number of participants
n_con <- unique(PreRegCon_long$ParticipantNumber)
...
After asking respondents to share their concerns in a free text format, we provided a 
list of possible concerns that researchers could have about preregistering their studies. 
They were able to select as many concerns as they liked. A total of `r length(n_con)`
respondents answered this question. Of those who respondended to this question, 
`r con[5,2]` reported that they do not share any of the listed concerns. 

```{r prereg_concerns_bar, echo=FALSE, warning=TRUE}
# build the bar chart

title = sprintf("Concerns about preregistering studies (n = %d)", length(n_con))
...
CodeCon_long <- df %>% 
  select (c("ParticipantNumber", "CodeCon_criticise", "CodeCon_diff_understand", "CodeCon_assistance", "CodeCon_lose_control", "CodeCon_errors", "CodeCon_credit", "CodeCon_no_con", "CodeCon_violate", "CodeCon_ip")) %>%  
...
# getting a tibble for the counts
code_con <- count(CodeCon_long$CodeConcern)

# need the data to figure out how many respondents shared concerns
  # figure out unique number of participants
n_code_con <- unique(CodeCon_long$ParticipantNumber)
  # count the number of unique values (can do that in the title itself in next chunk)
# length(n_con)
```

We also asked respondents if they have any concerns about making their research materials
and/or code open. After asking respondents to share their concerns in a free text format, we provided a list of possible concerns that researchers could have about sharing their materials
